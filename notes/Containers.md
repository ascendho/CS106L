...
6. 双端队列 `std::deque` 是一种双端队列结构，允许在两端高效地进行插入和删除操作

> 双端队列（deque）有着与向量（vector）相同的接口，只是它还允许执行 `push_front`（前端插入）和 `pop_front`（前端弹出）操作

7. 双端队列是如何实现的：采用数组的数组（即由多个子数组构成）的方式，各个独立的子数组会单独分配内存，如图所示：
...
`std::vector<T>` 的问题在于它采用**单一内存分配**：插入点之后的所有元素都必须移动（才能腾出空间）。`std::deque<T>` 则通过将内存分配**拆分为多个固定大小的内存分配块**，解决了这个问题。为了追踪 “已使用区域” 的起止位置，`std::deque<T>` 会使用 `start` 和 `finish` 索引 —— 实际上，这两个值是**迭代器**。`blocks` 是一个 “数组的数组”（即存储多个子数组地址的顶层数组），而这里的 `capacity` 指的是这个顶层数组的大小。
...
当我们尝试在 `deque` 开头插入多个元素，导致双端队列的第一个正在使用的块耗尽空间时，我们就必须分配一个新块。但在此之前，需要在**blocks 数组**中记录指向这个新分配块的指针 —— 而此时 `blocks` 数组本身也已没有空间。与 `std::vector` 在扩容时会将其元素数组容量翻倍的机制类似，双端队列对 `blocks` 数组也会执行相同逻辑的操作。一旦 `blocks` 数组完成扩容（复制所有旧指针，随后释放旧的 `blocks` 数组），就会分配一个新块来存储元素 。

![image-20250917204455526](assets/image-20250917204455526.png)

需要注意的是，本例中的 `start` 和 `finish` 是**相对于 blocks 数组的起始位置**而言的，所以 `start` 是 7（4+3，指向第8个元素），仿佛该数组已完全填满。但实际上，块是按需分配的，因此本例中的第一个块处于未分配状态，并在 `blocks` 数组中以空指针形式存储。
...
8. 如果 `std::deque<T>` 在块数组耗尽空间时必须将其扩容一倍，那它为何比 `std::vector<T>` 更高效呢？

> 实际上，deque 中每个块的大小会根据类型 T 有所不同，通常包含数百到数千个元素。这意味着，块数组的扩容操作实际上比 vector 中数据数组的扩容要快数百到数千倍。

9. 尽管 `deque` 的元素分散在多个分配的内存块中，但它仍像 `std::vector<T>` 一样支持通过索引访问元素
...
10. 关联式容器通过唯一键来组织元素，类似于 Python 中的字典和集合
11. `std::map`（映射容器）将键映射到值，有时也被称为关联数组
...
12. 为什么 `std::pair<const K, V>` 中 `K` 用 `const` 修饰？
...
13. 有序容器（`map` 和 `set`）会对其元素类型施加排序约束，从而能够实现 log 对数时间复杂度的键查找。在内部实现中，这些容器会通过一个比较函数对元素进行排序。因此，遍历 `map` 的键值对（或 `set` 的元素）时，得到的结果始终会按键（或集合元素）从小到大的顺序排列，如下例所示：
...
14. `std::map<K, V>` 要求键类型 `K` 具有小于运算符（`operator<`），如果 `K` 没有定义 `operator<`，则需要在声明 `std::map` 时显式指定自定义比较器（如通过第三个模板参数 `Compare`）
...
如果自定义类型 `MyType` 缺少小于运算符，只要你能定义该运算符，仍可将其用于 `map` 或 `set` 容器 —— 而定义该运算符有多种方式：

```c++
// 1.定义一个小于运算符（operator<）。
//   例如，可以编写一个非成员函数 operator<
...
// 2.定义一个函数对象（functor）。
//   如果不想为自定义类型 MyType 重载全局小于运算符（operator<），
//   那么推荐使用这种方法。
//   这种方式的原理是：向 map 和 set 显式传递一个比较模板参数；
//   若未将该参数作为模板参数提供，其默认值为std::less<T> 
//   这是 C++ 标准库中的一个内置函数对象（functor），
//   通过小于运算符（operator<）对键（key）进行比较
...
// 3.使用 Lambda 函数。
//   与上述方法类似，若你不想重载全局小于运算符（operator<），
// 	 这种方式会很实用，同时还能避免创建新的函数对象（functor）类型。
// 	 从功能上来说，它与第 2 种方法（定义函数对象）是相同的
...
// 注：decltype (comp) 会推导出 Lambda 函数 comp 的编译期类型（compile-time type），
//    该类型通常会因使用 auto 关键字而被隐藏。需注意，我们还需将该 Lambda 函数传递给 map 和 set 的构造函数。
```

15. 如果键 `k` 不存在，`m[k]` 会向 `map` 容器 `m` 中插入一个**默认初始化的值**！该默认值由值类型 V 的**无参构造函数**生成。例如，bool 类型的默认值是 false，int、size_t、float 和 double 类型的默认值是 0，而大多数容器类型的默认值是空容器。这种特性可能会导致一些奇怪的行为，比如即便你只是尝试读取某个值，对应的键也可能出现在 `map` 中。例如：
...
// 这段代码会输出 1，
// 尽管它看似只是从 map 中读取键 "Bjarne" 对应的值。
// 这种行为的原因在于，它能让某些算法的实现变得非常简单。
...
// 将此与等效的 Python 代码进行比较：在 Python 中，若要递增每个字符的计数，
// 可能需要先检查该字符是否存在于字典（map）中，然后再执行递增操作。
...
// 在 C++ 中，值的默认初始化省去了这种检查！
```

16. `std::set` 存储一组唯一的元素，可以看作是 “无值版的” `std::map`

> set 不存在键值对的概念，因此其底层的树节点（TreeNode）相对更简单：它并非存储键和值的对，而是直接将 set 的元素存储在节点中。除此之外，set 和 map 的工作原理完全相同：从概念上讲，你可以将 set 理解为一种 “没有任何值的 map”—— 即仅保留了 “键” 的特性（唯一性、可排序），却无需关联对应值的容器。

17. 你可以把 `unordered_map` 看作是 `map` 的优化版本，它具有与 `map` 相同的接口
...
18. `std::unordered_map<K, V>` 要求键类型 `K` 具备一个哈希函数以及（键的）相等性判断（机制）
...
20. 有趣的 C++ 小知识：`max_load_factor`（最大负载因子），你可以在执行重新哈希操作前，控制（`unordered_map`的）最大负载因子
...
21. `std::unordered_set`（无序集合容器）是没有值的 `std::unordered_map`（无序映射容器），不允许重复元素

22. 要使用 `std::unordered_map<K, V>` 或 `std::unordered_set<T>`，类型  `K` 或类型 `T` 必须具备关联的哈希函数和相等性函数，可参考上文为 `map` 定义小于运算符的三种方式

23. 何时使用 `unordered_map` 与 `map`？
